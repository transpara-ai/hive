...
if docker exec hive-postgres-1 pg_isready -U hive -q 2>/dev/null; then
  ...
  if [ "$hive_state" = "active" ] || [ "$hive_state" = "reloading" ]; then
    ...
    # unit encodes — a PROTECTED ACTION (see the human gate in On-demand
    # Runtime): require the user's explicit current-turn approval naming the
    # credential AND autonomy postures, then: systemctl --user restart hive
