...
intake channel, stage ledger, elapsed time, peers, gate state, evidence,
`human_approval_basis` (`standing_scoped` or `fresh_scoped`) with its receipt,
blocker, next action, budget, PR identity/head/check state, and the single
open-intervention list.
...
A supervisor owns process IDs/logs and exact executable/config hashes. All
listed services use the fresh v1 configuration and data roots. Preflight fails
closed if a listed port is bound by a process outside the supervisor's exact
PID/executable-hash set; it never adopts an existing listener. It uses fresh
mode-0600 runtime configuration and never reads the tracked
...
| R11 | acceptance harness | `TestFactoryV1Operation86Immutability` |
| R12 | supervisor/config | `TestFactoryV1SecretSafeIsolation` |
