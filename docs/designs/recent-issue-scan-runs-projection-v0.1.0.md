...
- **status:** post-CFAR round 2 (PR #241)
...
### CFAR round 2 (codex, on PR #241) — 1 blocker → resolved

Follow-up implementation-stage adversarial review on the CFAR round 1 fix itself, targeting the `workEvidenceQueryFailed` signal that `civilizationRecentIssueScanRuns` relies on to withhold `queued`/`in_flight` promotion.

- **Partial Work-evidence read failure not propagated (P2):** `civilizationAssemblyFactoryOrders` only set its returned `factoryOrdersQueryFailed` (renamed internally to `workEvidenceQueryFailed`) when the `work.task.created` page fetch itself failed. But the function also performs four more Work reads while assembling `factoryOrderWorkEvidence` — the task-artifact/dependency/lifecycle-transition/verification `ByType` queries — plus a per-task `civilizationAssemblyProjectWorkTask` projection (which itself reads lifecycle transitions) for every task-created event. When any ONE of those failed (for example `work.task.lifecycle.transitioned` erroring while the `work.task.created` page succeeded), the error was only appended to `operatorProjection.Errors`/`FailureReasons` as text — the boolean stayed `false`, and the affected task was silently omitted from `factoryOrderWorkEvidence.Tasks`. `workEvidenceUnprovable` in the fold (`workEvidenceQueryFailed || workEvidenceTruncated`) therefore stayed `false` too, and a requested run for that factory order was promoted to `queued` on the strength of "no stage work evidence found" — when the truth was "the read that would have found it failed." Resolved: `civilizationAssemblyFactoryOrders` now threads a single fail-closed boolean through the whole computation — it starts `false` and is OR'd `true` by each of the five sub-read failure sites (artifact, dependency, lifecycle, verification `ByType` errors, and the per-task `civilizationAssemblyProjectWorkTask` error) — so `workEvidenceQueryFailed` is an ALLOWLIST: `true` (failed) unless every sub-read is proven to have succeeded. No string-matching of `FailureReasons`; the flag is computed at the read site and threaded out. The fold's own wiring (`workEvidenceUnprovable := workEvidenceQueryFailed || workEvidenceTruncated`, degrade to `recorded`) was already correct from CFAR round 1 — only the source signal needed completing.
  - The doc comment on `civilizationRecentIssueScanRuns` now states the completed evidence-uncertainty matrix explicitly: parked page (unreadable → whole rail unavailable; truncated → per-run degrade to recorded when unmatched), requested page (unreadable → whole rail unavailable; truncated → affected rows are simply absent from `requestedEvents`, not misclassified), work evidence (ANY read failure or truncation across all five sub-reads → every requested run otherwise eligible for in_flight/queued degrades to recorded — no promotion from an unproven-complete computation).
  - Test: `TestCivilizationRecentIssueScanRunsPartialWorkEvidenceReadFailureDegradesToRecorded` — a store wrapper failing ONLY `work.task.lifecycle.transitioned` `ByType` reads while every other read (including the `work.task.created` page and the factory order's own stage task-created event) succeeds. Before the fix the run came out `queued`; after, `recorded`.
